...
const DEFAULT_GLOW_COLOR = '255, 92, 20';
...
const cardData = [
  {
    color: '#ffffff',
    title: 'Front-End Dev',
    description: 'React, responsive UIs, clean component architecture',
    label: 'Build'
  },
  {
    color: '#ffffff',
    title: 'Web Design',
    description: 'Figma to pixel-perfect, modern interfaces',
    label: 'Design'
  },
  {
    color: '#ffffff',
    title: 'Full-Stack',
    description: 'Flask & Firebase back ends, REST APIs, auth',
    label: 'Ship'
  },
  {
    color: '#ffffff',
    title: 'Motion & UX',
    description: 'Micro-interactions and animation that feel alive',
    label: 'Polish'
  },
  {
    color: '#ffffff',
    title: 'Deploy',
    description: 'Vercel, Netlify, CI/CD — from local to live',
    label: 'Launch'
  }
];
...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    // Pointer events so a finger press/drag triggers particles + effects on touch.
    element.addEventListener('pointerenter', handleMouseEnter);
    element.addEventListener('pointerleave', handleMouseLeave);
    element.addEventListener('pointercancel', handleMouseLeave);
    ...
    return () => {
      ...
      element.removeEventListener('pointercancel', handleMouseLeave);
...
const MagicBento = ({
  ...
  // On touch we KEEP the glow, particles, border glow and click ripple, but drop
  // the 3D tilt + magnetism — those follow a hovering cursor and would fight
  // the page scroll on a phone.
